                                    ...
                                    const gchar* symname)
{
	...
	if (_tmp5_) {
		...
		_tmp24_ = _tmp13_[_tmp14__length1 - 2];
		_g_free0 (_tmp19_);
		_tmp25_ = path;
		_tmp25__length1 = path_length1;
		_tmp26_ = _tmp25__length1 - 1;
		path = g_renew (gchar*, path, _tmp25__length1 - 1);
		(_tmp26_ > path_length1) ? memset (path + path_length1, 0, sizeof (gchar*) * (_tmp26_ - path_length1)) : NULL;
		path_length1 = _tmp26_;
		_path_size_ = _tmp26_;
		_tmp27_ = path;
		_tmp27__length1 = path_length1;
		_tmp28_ = valadoc_api_tree_search_symbol_path (self, element, _tmp27_, _tmp27__length1);
		result = _tmp28_;
		_g_object_unref0 (node);
		path = (_vala_array_free (path, path_length1, (GDestroyNotify) g_free), NULL);
		return result;
