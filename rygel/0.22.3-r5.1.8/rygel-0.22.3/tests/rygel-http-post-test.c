/* rygel-http-post-test.c generated by valac 0.24.0.112-bbf5, the Vala compiler
...
RygelTestRequestFactory* rygel_test_request_factory_new (SoupMessage* msg, SoupStatus* expected_code, gboolean cancel);
RygelTestRequestFactory* rygel_test_request_factory_construct (GType object_type, SoupMessage* msg, SoupStatus* expected_code, gboolean cancel);
static SoupStatus* _soup_status_dup (SoupStatus* self);
...
static void __lambda8_ (Block1Data* _data1_, SoupSession* session, SoupMessage* msg);
...
static SoupStatus* _soup_status_dup (SoupStatus* self) {
	SoupStatus* dup;
	dup = g_new0 (SoupStatus, 1);
	memcpy (dup, self, sizeof (SoupStatus));
	return dup;
}


static gpointer __soup_status_dup0 (gpointer self) {
	return self ? _soup_status_dup (self) : NULL;
}
...
RygelTestRequestFactory* rygel_test_request_factory_construct (GType object_type, SoupMessage* msg, SoupStatus* expected_code, gboolean cancel) {
	...
	SoupStatus* _tmp2_ = NULL;
	SoupStatus* _tmp3_ = NULL;
	...
	_tmp3_ = __soup_status_dup0 (_tmp2_);
...
static gboolean _rygel_http_post_test_on_timeout_gsource_func (gpointer self) {
	gboolean result;
	result = rygel_http_post_test_on_timeout ((RygelHTTPPostTest*) self);
	return result;
}


static void _rygel_http_post_test_on_message_received_rygel_http_server_message_received (RygelHTTPServer* _sender, SoupMessage* message, gpointer self) {
	rygel_http_post_test_on_message_received ((RygelHTTPPostTest*) self, _sender, message);
}


static void _rygel_http_post_test_on_client_completed_rygel_state_machine_completed (RygelStateMachine* _sender, gpointer self) {
	rygel_http_post_test_on_client_completed ((RygelHTTPPostTest*) self, _sender);
}
...
static void rygel_http_post_test_real_run (RygelHTTPPostTest* self, GError** error) {
	...
	g_remove (_tmp3_);
	_g_free0 (_tmp3_);
	g_timeout_add_seconds_full (G_PRIORITY_DEFAULT, (guint) 10, _rygel_http_post_test_on_timeout_gsource_func, g_object_ref (self), g_object_unref);
	_tmp4_ = self->server;
...
static gboolean rygel_http_post_test_check_result_co (RygelHttpPostTestCheckResultData* _data_) {
		...
		}
		{
			_data_->i = 0;
			{
				...
				while (TRUE) {
					...
					if (!(((gsize) _data_->_tmp12_) < RYGEL_HTTP_CLIENT_LENGTH)) {
						break;
					}
					_data_->_tmp13_ = NULL;
					...
					_data_->_tmp16_ = NULL;
					_data_->_tmp16_ = _data_->self->client;
					_data_->_tmp17_ = NULL;
					_data_->_tmp17__length1 = 0;
					_data_->_tmp17_ = _data_->_tmp16_->content;
					_data_->_tmp17__length1 = _data_->_tmp16_->content_length1;
					_data_->_tmp18_ = 0;
					_data_->_tmp18_ = _data_->i;
					_data_->_tmp19_ = 0U;
					_data_->_tmp19_ = _data_->_tmp17_[_data_->_tmp18_];
					_vala_assert (_data_->_tmp15_ == _data_->_tmp19_, "buffer[i] == this.client.content[i]");
				}
			}
		}
		_data_->buffer = (g_free (_data_->buffer), NULL);
	...
	__catch2_g_error:
	{
		...
		_g_error_free0 (_data_->self->priv->error);
		_data_->self->priv->error = _data_->_tmp21_;
		_data_->_tmp22_ = NULL;
		_data_->_tmp22_ = _data_->self->priv->main_loop;
...
RygelHTTPServer* rygel_http_server_construct (GType object_type, GError** error) {
	...
	_tmp13_ = _tmp12_;
	g_signal_connect_object (_tmp13_, "request-started", (GCallback) _rygel_http_server_on_request_started_soup_server_request_started, self, 0);
...
static gboolean rygel_http_client_real_run_co (RygelHttpClientRunData* _data_) {
	...
	soup_session_queue_message (_data_->_tmp5_, _data_->_tmp7_, ___lambda8__soup_session_callback, block1_data_ref (_data_->_data1_));
	_data_->_state_ = 1;
	return FALSE;
	_state_1:
	;
...
static void _rygel_media_container_on_file_changed_g_file_monitor_changed (GFileMonitor* _sender, GFile* file, GFile* other_file, GFileMonitorEvent event_type, gpointer self) {
	rygel_media_container_on_file_changed ((RygelMediaContainer*) self, _sender, file, other_file, event_type);
}
...
RygelMediaContainer* rygel_media_container_construct (GType object_type) {
	...
	g_signal_connect_object (_tmp7_, "changed", (GCallback) _rygel_media_container_on_file_changed_g_file_monitor_changed, self, 0);
	return self;
}
...
static gboolean rygel_media_container_find_object_co (RygelMediaContainerFindObjectData* _data_) {
	...
	if (_data_->_tmp0_) {
		_data_->_tmp3_ = NULL;
		_data_->_tmp3_ = _ ("Fake error caused by %s object.");
		_data_->_tmp4_ = NULL;
		_data_->_tmp4_ = g_strdup (_data_->_tmp3_);
		_data_->msg = _data_->_tmp4_;
		_data_->_tmp5_ = NULL;
		_data_->_tmp5_ = _data_->self->file;
		...
		_data_->_tmp7_ = NULL;
		_data_->_tmp7_ = _data_->item_id;
		_data_->_tmp8_ = NULL;
		_data_->_tmp8_ = g_error_new (RYGEL_CONTENT_DIRECTORY_ERROR, RYGEL_CONTENT_DIRECTORY_ERROR_INVALID_ARGS, _data_->_tmp6_, _data_->_tmp7_);
		...
		_data_->_tmp10_ = NULL;
		_data_->_tmp10_ = _data_->item_id;
		if (g_strcmp0 (_data_->_tmp10_, "ErrorItem") == 0) {
			_data_->_tmp11_ = FALSE;
			_data_->_tmp11_ = _data_->self->priv->error;
	...
	_data_->_tmp13_ = NULL;
	_data_->_tmp13_ = _data_->item_id;
	if (g_strcmp0 (_data_->_tmp13_, "VanishingItem") == 0) {
		_data_->_tmp14_ = FALSE;
		_data_->_tmp14_ = _data_->self->priv->vanish;
	...
	if (_data_->_tmp12_) {
		_data_->_tmp15_ = NULL;
		_data_->_tmp15_ = _data_->self->file;
		g_file_delete (_data_->_tmp15_, NULL, &_data_->_inner_error_);
		if (G_UNLIKELY (_data_->_inner_error_ != NULL)) {
			g_simple_async_result_set_from_error (_data_->_async_result, _data_->_inner_error_);
			g_error_free (_data_->_inner_error_);
			block2_data_unref (_data_->_data2_);
		...
		_data_->_tmp17_ = NULL;
		_data_->_tmp17_ = _data_->item_id;
		if (g_strcmp0 (_data_->_tmp17_, "VanishingItem") == 0) {
			_data_->_tmp18_ = FALSE;
			_data_->_tmp18_ = _data_->self->priv->vanish;
	...
	if (g_strcmp0 (_data_->_tmp19_, _data_->_tmp21_) != 0) {
		_data_->_tmp22_ = NULL;
		_data_->_tmp22_ = _data_->item_id;
		_data_->_tmp23_ = NULL;
		_data_->_tmp23_ = rygel_media_item_new (_data_->_tmp22_, _data_->self);
		_g_object_unref0 (_data_->self->item);
		_data_->self->item = _data_->_tmp23_;
	}
	_data_->_tmp24_ = NULL;
	_data_->_tmp24_ = _data_->self->item;
	_data_->_tmp25_ = NULL;
	_data_->_tmp25_ = _g_object_ref0 ((RygelMediaObject*) _data_->_tmp24_);
...
static gboolean rygel_http_response_real_run_co (RygelHttpResponseRunData* _data_) {
	...
	g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, ___lambda9__gsource_func, block4_data_ref (_data_->_data4_), block4_data_unref);
	_data_->_state_ = 1;
	return FALSE;
	_state_1:
	;
