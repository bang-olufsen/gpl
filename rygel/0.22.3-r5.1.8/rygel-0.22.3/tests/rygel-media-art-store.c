/* rygel-media-art-store.c generated by valac 0.24.0.112-bbf5, the Vala compiler
...
RygelMediaArtStore* rygel_media_art_store_get_default (void) {
	...
	if (_tmp0_) {
		...
		__catch0_media_art_store_error:
		{
			...
			g_warning ("rygel-media-art-store.vala:54: No media art available: %s", _tmp5_);
...
static RygelMediaArtStore* rygel_media_art_store_construct (GType object_type, GError** error) {
	...
	self->priv->directory = _tmp8_;
	{
		...
		self->priv->char_remove_regex = _tmp16_;
		_tmp17_ = g_regex_escape_string (RYGEL_MEDIA_ART_STORE_CONVERT_CHARS, -1);
		_g_free0 (regex_string);
		regex_string = _tmp17_;
		_tmp19_ = regex_string;
		...
		self->priv->char_convert_regex = _tmp24_;
		_tmp26_ = g_regex_new ("\\s+", 0, 0, &_inner_error_);
		_tmp25_ = _tmp26_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
		...
		self->priv->space_compress_regex = _tmp27_;
		_tmp28_ = g_new0 (GRegex*, 0 + 1);
		...
		self->priv->block_regexes = _tmp28_;
		...
		self->priv->_block_regexes_size_ = self->priv->block_regexes_length1;
		{
			...
			for (block_it = 0; block_it < G_N_ELEMENTS (RYGEL_MEDIA_ART_STORE_BLOCKS); block_it = block_it + 1) {
				...
				block = block_collection[block_it];
				{
					...
					_tmp38_ = _tmp37_;
					_tmp39_ = g_regex_escape_string (_tmp38_, -1);
					_tmp40_ = _tmp39_;
					_tmp41_ = block;
					_tmp42_ = string_get (_tmp41_, (glong) 1);
					_tmp43_ = g_strdup_printf ("%C", _tmp42_);
					_tmp44_ = _tmp43_;
					_tmp45_ = g_regex_escape_string (_tmp44_, -1);
					_tmp46_ = _tmp45_;
					_tmp47_ = g_strdup_printf (RYGEL_MEDIA_ART_STORE_BLOCK_PATTERN, _tmp34_, _tmp40_, _tmp46_);
					_tmp48_ = _tmp47_;
					_g_free0 (_tmp46_);
					_g_free0 (_tmp44_);
					_g_free0 (_tmp40_);
					...
					block_re = _tmp48_;
					_tmp50_ = block_re;
					_tmp51_ = g_regex_new (_tmp50_, 0, 0, &_inner_error_);
					_tmp49_ = _tmp51_;
					if (G_UNLIKELY (_inner_error_ != NULL)) {
						_g_free0 (block_re);
					...
					_tmp52_ = self->priv->block_regexes;
					_tmp52__length1 = self->priv->block_regexes_length1;
					_tmp53_ = _tmp49_;
					_tmp49_ = NULL;
					_vala_array_add1 (&self->priv->block_regexes, &self->priv->block_regexes_length1, &self->priv->_block_regexes_size_, _tmp53_);
...
static gchar* rygel_media_art_store_normalize_and_hash (RygelMediaArtStore* self, const gchar* input, gboolean utf8_only) {
	...
	if (_tmp1_) {
		...
		if (_tmp4_) {
			...
			gchar* _tmp10_ = NULL;
			_tmp7_ = input;
			_tmp8_ = rygel_media_art_store_strip_invalid_entities (self, _tmp7_);
			_g_free0 (normalized);
			normalized = _tmp8_;
			_tmp9_ = normalized;
		...
		_tmp11_ = normalized;
		_tmp12_ = g_utf8_normalize (_tmp11_, (gssize) (-1), G_NORMALIZE_ALL);
		_g_free0 (normalized);
		normalized = _tmp12_;
	}
	_tmp13_ = normalized;
...
static gchar* rygel_media_art_store_strip_invalid_entities (RygelMediaArtStore* self, const gchar* original) {
	...
	p = _tmp1_;
	{
		...
		_tmp10_ = self->priv->char_remove_regex;
		_tmp11_ = p;
		...
		_g_free0 (p);
		p = _tmp13_;
		_tmp15_ = self->priv->char_convert_regex;
		_tmp16_ = p;
		_tmp17_ = g_regex_replace_literal (_tmp15_, _tmp16_, (gssize) (-1), 0, " ", 0, &_inner_error_);
		_tmp14_ = _tmp17_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			_g_free0 (_tmp9_);
			if (_inner_error_->domain == G_REGEX_ERROR) {
		...
		_g_free0 (p);
		p = _tmp18_;
		_tmp20_ = self->priv->space_compress_regex;
		_tmp21_ = p;
		_tmp22_ = g_regex_replace_literal (_tmp20_, _tmp21_, (gssize) (-1), 0, " ", 0, &_inner_error_);
		_tmp19_ = _tmp22_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			_g_free0 (_tmp14_);
			_g_free0 (_tmp9_);
