/* rygel-playbin-player.c generated by valac 0.24.0, the Vala compiler
...
static void __lambda3_ (RygelPlaybinPlayer* self, GUPnPDIDLLiteObject* object);
static void _g_object_unref0_ (gpointer var);
static void _g_list_free__g_object_unref0_ (GList* self);
static void ___lambda3__gupnp_didl_lite_parser_object_available (GUPnPDIDLLiteParser* _sender, GUPnPDIDLLiteObject* object, gpointer self);
...
RygelPlaybinPlayer* rygel_playbin_player_construct_wrap (GType object_type, GstElement* playbin) {
	...
	g_return_val_if_fail (g_strcmp0 (_tmp2_, "GstPlayBin") == 0, NULL);
	self = (RygelPlaybinPlayer*) g_object_new (object_type, NULL);
	_tmp3_ = playbin;
...
static gboolean rygel_playbin_player_real_seek (RygelMediaPlayer* base, gint64 time) {
	...
	g_debug ("rygel-playbin-player.vala:391: Seeking %lld usec, play speed %s", _tmp0_, _tmp1_);
	_tmp2_ = time;
...
static gboolean rygel_playbin_player_real_seek_bytes (RygelMediaPlayer* base, gint64 bytes) {
	...
	_tmp0_ = bytes;
	_tmp1_ = self->priv->_new_playback_speed;
	g_debug ("rygel-playbin-player.vala:403: Seeking %lld bytes, play speed %s", _tmp0_, _tmp1_);
...
static void rygel_playbin_player_bus_handler (RygelPlaybinPlayer* self, GstBus* bus, GstMessage* message) {
	...
	switch (_tmp1_) {
		...
		case GST_MESSAGE_EOS:
		{
			...
			if (!_tmp34_) {
				g_debug ("rygel-playbin-player.vala:549: EOS");
				rygel_media_player_set_playback_state ((RygelMediaPlayer*) self, "EOS");
			} else {
				g_debug ("rygel-playbin-player.vala:552: Content is image, ignoring EOS");
			}
			break;
		}
		case GST_MESSAGE_ERROR:
		{
			...
			g_warning ("rygel-playbin-player.vala:562: Error from GStreamer element %s: %s (%s" \
")", _tmp41_, _tmp43_, _tmp44_);
			_g_free0 (_tmp41_);
			g_warning ("rygel-playbin-player.vala:566: Going to STOPPED state");
			rygel_media_player_set_playback_state ((RygelMediaPlayer*) self, "STOPPED");
			_g_free0 (debug_message);
			_g_error_free0 (_error_);
			break;
...
static void rygel_playbin_player_on_source_setup (RygelPlaybinPlayer* self, GstElement* pipeline, GstElement* source) {
	...
	if (_tmp0_) {
		...
		g_debug ("rygel-playbin-player.vala:577: Setting transfer mode to %s", _tmp5_);
...
static void ___lambda3__gupnp_didl_lite_parser_object_available (GUPnPDIDLLiteParser* _sender, GUPnPDIDLLiteObject* object, gpointer self) {
	__lambda3_ ((RygelPlaybinPlayer*) self, object);
}
...
static void rygel_playbin_player_guess_duration (RygelPlaybinPlayer* self) {
	...
	reader = _tmp3_;
	_tmp4_ = reader;
	g_signal_connect_object (_tmp4_, "object-available", (GCallback) ___lambda3__gupnp_didl_lite_parser_object_available, self, 0);
	{
	...
	__catch0_g_error:
	{
		...
		g_debug ("rygel-playbin-player.vala:646: Failed to parse meta-data: %s", _tmp8_);
		_g_error_free0 (_error_);
	}
	__finally0:
	if (_inner_error_ != NULL) {
...
static void rygel_playbin_player_real_set_uri (RygelMediaPlayer* base, const gchar* value) {
	...
	if (g_strcmp0 (_tmp5_, "") != 0) {
		...
		gchar* _tmp13_ = NULL;
		_tmp13_ = g_strdup ("NO_MEDIA_PRESENT");
		_g_free0 (self->priv->_playback_state);
		self->priv->_playback_state = _tmp13_;
		g_object_notify ((GObject*) self, "playback-state");
	}
	_tmp14_ = value;
	g_debug ("rygel-playbin-player.vala:212: URI set to %s.", _tmp14_);
	g_object_notify ((GObject *) self, "uri");
}
...
static void rygel_playbin_player_real_set_volume (RygelMediaPlayer* base, gdouble value) {
	...
	_tmp2_ = value;
	g_debug ("rygel-playbin-player.vala:288: volume set to %f.", _tmp2_);
...
static gint64 rygel_playbin_player_real_get_byte_position (RygelMediaPlayer* base) {
	...
	self = (RygelPlaybinPlayer*) base;
	_tmp0_ = self->priv->_playbin;
	_tmp1_ = _dynamic_get_source10 (_tmp0_);
	_tmp2_ = _tmp1_;
