/* rygel-preferences-dialog.c generated by valac 0.24.0.112-bbf5, the Vala compiler
...
static void __lambda8_ (RygelPreferencesDialog* self);
...
static void ___lambda8__g_object_notify (GObject* _sender, GParamSpec* pspec, gpointer self) {
	__lambda8_ ((RygelPreferencesDialog*) self);
}
...
RygelPreferencesDialog* rygel_preferences_dialog_construct (GType object_type, GError** error) {
	...
	RygelWritableUserConfig* _tmp15_ = NULL;
	gboolean _tmp16_ = FALSE;
	GeeArrayList* _tmp17_ = NULL;
	RygelNetworkPrefSection* _tmp18_ = NULL;
	GtkBuilder* _tmp19_ = NULL;
	RygelWritableUserConfig* _tmp20_ = NULL;
	RygelNetworkPrefSection* _tmp21_ = NULL;
	...
	gtk_builder_add_from_file (_tmp4_, RYGEL_PREFERENCES_DIALOG_UI_FILE, &_inner_error_);
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
	...
	_tmp6_ = gtk_builder_get_object (_tmp5_, RYGEL_PREFERENCES_DIALOG_DIALOG);
	_tmp7_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_CAST (_tmp6_, GTK_TYPE_DIALOG, GtkDialog));
	_g_object_unref0 (self->priv->dialog);
	self->priv->dialog = _tmp7_;
	_tmp8_ = self->priv->dialog;
	_vala_assert (_tmp8_ != NULL, "this.dialog != null");
	_tmp9_ = self->priv->builder;
	_tmp10_ = gtk_builder_get_object (_tmp9_, RYGEL_PREFERENCES_DIALOG_UPNP_CHECKBUTTON);
	_tmp11_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_CAST (_tmp10_, GTK_TYPE_SWITCH, GtkSwitch));
	_g_object_unref0 (self->priv->upnp_check);
	self->priv->upnp_check = _tmp11_;
	_tmp12_ = self->priv->upnp_check;
	_vala_assert (_tmp12_ != NULL, "this.upnp_check != null");
	_tmp13_ = self->priv->dialog;
	gtk_window_set_icon_from_file ((GtkWindow*) _tmp13_, RYGEL_PREFERENCES_DIALOG_ICON, &_inner_error_);
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
		_g_object_unref0 (_tmp0_);
		_g_object_unref0 (self);
	...
	_tmp14_ = self->priv->upnp_check;
	_tmp15_ = self->priv->config;
	_tmp16_ = rygel_writable_user_config_is_upnp_enabled (_tmp15_);
	gtk_switch_set_active (_tmp14_, _tmp16_);
	_tmp17_ = gee_array_list_new (RYGEL_TYPE_PREFERENCES_SECTION, (GBoxedCopyFunc) g_object_ref, g_object_unref, NULL, NULL, NULL);
	_g_object_unref0 (self->priv->sections);
	self->priv->sections = _tmp17_;
	_tmp19_ = self->priv->builder;
	_tmp20_ = self->priv->config;
	_tmp21_ = rygel_network_pref_section_new (_tmp19_, _tmp20_, &_inner_error_);
	_tmp18_ = _tmp21_;
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
		_g_object_unref0 (_tmp0_);
	...
	_tmp22_ = self->priv->sections;
	gee_abstract_collection_add ((GeeAbstractCollection*) _tmp22_, (RygelPreferencesSection*) _tmp18_);
	...
	g_signal_connect_object ((GObject*) _tmp28_, "notify::active", (GCallback) ___lambda8__g_object_notify, self, 0);
...
gint rygel_preferences_dialog_main (gchar** args, int args_length1) {
	...
	gtk_init (&args_length1, &args);
	{
		...
		RygelPreferencesDialog* _tmp2_ = NULL;
		...
		rygel_meta_config_register_configuration ((RygelConfiguration*) _tmp0_);
		_tmp2_ = rygel_preferences_dialog_new (&_inner_error_);
		dialog = _tmp2_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			_g_object_unref0 (_tmp0_);
			goto __catch0_g_error;
		...
		_tmp3_ = dialog;
		rygel_preferences_dialog_run (_tmp3_);
	...
	__catch0_g_error:
	{
		...
		err = _inner_error_;
		_inner_error_ = NULL;
		_tmp4_ = _ ("Failed to create preferences dialog: %s");
		_tmp5_ = err->message;
		g_error (_tmp4_, _tmp5_);
		_g_error_free0 (err);
	}
	__finally0:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
