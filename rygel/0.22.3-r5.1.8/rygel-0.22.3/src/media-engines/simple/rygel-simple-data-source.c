/* rygel-simple-data-source.c generated by valac 0.24.0, the Vala compiler
...
static gboolean __lambda4_ (RygelSimpleDataSource* self);
...
static gboolean ___lambda4__gsource_func (gpointer self) {
	gboolean result;
	result = __lambda4_ ((RygelSimpleDataSource*) self);
	return result;
}
...
"ile %s", _tmp2_);
	fd = -1;
	{
		...
		while (TRUE) {
			...
			_tmp35_ = start;
			_tmp36_ = G_MAXUINT16;
			stop = _tmp35_ + _tmp36_;
			_tmp37_ = stop;
			_tmp38_ = self->priv->last_byte;
			if (_tmp37_ > _tmp38_) {
				off_t _tmp39_ = {0};
				_tmp39_ = self->priv->last_byte;
				stop = _tmp39_;
			}
			_tmp40_ = stop;
			_tmp41_ = start;
			_tmp42_ = g_new0 (guint8, _tmp40_ - _tmp41_);
			_data1_->slice = _tmp42_;
			_data1_->slice_length1 = _tmp40_ - _tmp41_;
			_data1_->_slice_size_ = _data1_->slice_length1;
			_tmp43_ = fd;
			_tmp44_ = _data1_->slice;
			_tmp44__length1 = _data1_->slice_length1;
			_tmp45_ = _data1_->slice;
			_tmp45__length1 = _data1_->slice_length1;
			_tmp46_ = read (_tmp43_, _tmp44_, (gsize) _tmp45__length1);
			len = (gint) _tmp46_;
			_tmp47_ = len;
			if (_tmp47_ < 0) {
				GFile* _tmp48_ = NULL;
				...
				_tmp50_ = _tmp49_;
				_tmp51_ = errno;
				_tmp52_ = strerror (_tmp51_);
				_tmp53_ = g_error_new (G_IO_ERROR, G_IO_ERROR_FAILED, "Failed to read file '%s': %s", _tmp50_, _tmp52_);
				_tmp54_ = _tmp53_;
				_g_free0 (_tmp50_);
			...
			_tmp55_ = len;
			_data1_->slice_length1 = _tmp55_;
			_tmp56_ = _data1_->slice_length1;
			_tmp57_ = stop;
			self->priv->first_byte = _tmp57_;
			g_idle_add_full (G_PRIORITY_DEFAULT_IDLE, _____lambda3__gsource_func, block1_data_ref (_data1_), block1_data_unref);
			block1_data_unref (_data1_);
			_data1_ = NULL;
		}
	}
	...
	__catch0_g_error:
	{
		...
		g_warning ("rygel-simple-data-source.vala:171: Failed to stream file %s: %s", _tmp60_, _tmp62_);
		_g_free0 (_tmp60_);
