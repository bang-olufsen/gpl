/* rygel-gst-sink.c generated by valac 0.24.0, the Vala compiler
...
static GstFlowReturn rygel_gst_sink_real_render (GstBaseSink* base, GstBuffer* buffer) {
	...
	_tmp8_ = self->priv->priority;
	g_idle_add_full (_tmp8_, ___lambda3__gsource_func, block1_data_ref (_data1_), block1_data_unref);
	result = GST_FLOW_OK;
	block1_data_unref (_data1_);
	_data1_ = NULL;
	return result;
...
gboolean rygel_gst_sink_push_data (RygelGstSink* self, GstBuffer* buffer) {
	...
	gint64 _tmp10_ = 0LL;
	GstMapInfo info = {0};
	GstBuffer* _tmp11_ = NULL;
	GstMapInfo _tmp12_ = {0};
	guint8* tmp = NULL;
	...
	gint64 _tmp18_ = 0LL;
	gint64 _tmp19_ = 0LL;
	GstBuffer* _tmp20_ = NULL;
	GstMapInfo _tmp21_ = {0};
	...
	_tmp6_ = buffer;
	_tmp7_ = gst_buffer_get_size (_tmp6_);
	bufsize = _tmp7_;
	_tmp8_ = bufsize;
	_tmp9_ = left;
	...
	to_send = _tmp10_;
	_tmp11_ = buffer;
	gst_buffer_map (_tmp11_, &_tmp12_, GST_MAP_READ);
	info = _tmp12_;
	_tmp13_ = info;
	_tmp14_ = _tmp13_.data;
	_tmp14__length1 = (gint) _tmp13_.size;
	_tmp15_ = to_send;
	tmp = _tmp14_ + 0;
	tmp_length1 = ((gint) _tmp15_) - 0;
	_tmp_size_ = tmp_length1;
	_tmp16_ = self->priv->source;
	_tmp17_ = tmp;
	_tmp17__length1 = tmp_length1;
	g_signal_emit_by_name (_tmp16_, "data-available", _tmp17_, _tmp17__length1);
	_tmp18_ = self->priv->bytes_sent;
	_tmp19_ = to_send;
	self->priv->bytes_sent = _tmp18_ + _tmp19_;
	_tmp20_ = buffer;
	_tmp21_ = info;
	gst_buffer_unmap (_tmp20_, &_tmp21_);
	result = FALSE;
	return result;
}
