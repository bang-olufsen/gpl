/* rygel-gst-media-engine.c generated by valac 0.24.0, the Vala compiler
			...
			while (TRUE) {
				...
				if (_tmp38_ == ((0 != _tmp37_label0) ? _tmp37_label0 : (_tmp37_label0 = g_quark_from_static_string ("lpcm")))) {
					switch (0) {
						default:
						{
							RygelL16Transcoder* _tmp39_ = NULL;
					...
					switch (0) {
						default:
						{
							RygelMP3Transcoder* _tmp40_ = NULL;
					...
					switch (0) {
						default:
						{
							...
							RygelMP2TSTranscoder* _tmp43_ = NULL;
					...
					switch (0) {
						default:
						{
							RygelWMVTranscoder* _tmp44_ = NULL;
					...
					switch (0) {
						default:
						{
							RygelAACTranscoder* _tmp45_ = NULL;
					...
					switch (0) {
						default:
						{
							RygelAVCTranscoder* _tmp46_ = NULL;
							_tmp46_ = rygel_avc_transcoder_new ();
							self->priv->transcoders = g_list_prepend (self->priv->transcoders, (RygelTranscoder*) _tmp46_);
							break;
						}
					}
				} else {
					switch (0) {
						default:
						{
							...
							_tmp47_ = transcoder;
							g_debug ("rygel-gst-media-engine.vala:102: Unsupported transcoder \"%s\"", _tmp47_);
							break;
						}
					}
				}
...
static RygelDataSource* rygel_gst_media_engine_real_create_data_source (RygelMediaEngine* base, const gchar* uri) {
	...
	g_return_val_if_fail (uri != NULL, NULL);
	{
		...
		RygelGstDataSource* _tmp3_ = NULL;
		_tmp1_ = uri;
	...
	__catch1_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp4_ = _ ("Failed to create GStreamer data source for %s: %s");
		_tmp5_ = uri;
		_tmp6_ = _error_;
