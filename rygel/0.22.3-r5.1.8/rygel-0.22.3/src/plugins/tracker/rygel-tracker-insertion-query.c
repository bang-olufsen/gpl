/* rygel-tracker-insertion-query.c generated by valac 0.24.0, the Vala compiler
...
static gboolean rygel_tracker_insertion_query_real_execute_co (RygelTrackerInsertionQueryExecuteData* _data_) {
	...
	if (_data_->_tmp13_ == NULL) {
		...
		if (_data_->_tmp27_) {
			...
			_data_->_tmp33_ = NULL;
			_data_->_tmp33_ = _data_->miner;
			_data_->_tmp34_ = NULL;
			_data_->_tmp34_ = _data_->self->priv->uri;
			rygel_tracker_miner_files_index_iface_index_file (_data_->_tmp33_, _data_->_tmp34_, NULL, NULL);
			_g_object_unref0 (_data_->miner);
		}
		_g_object_unref0 (_data_->file);
	}
	_data_->_tmp35_ = NULL;
	_data_->_tmp35_ = _data_->self->id;
	g_debug ("rygel-tracker-insertion-query.vala:167: Created item in Tracker store " \
"with ID '%s'", _data_->_tmp35_);
	_g_free0 (_data_->key);
	_g_variant_iter_free0 (_data_->iter3);
	_g_variant_iter_free0 (_data_->iter2);
	_g_variant_iter_free0 (_data_->iter1);
