/* rygel-mpris-plugin-factory.c generated by valac 0.24.0, the Vala compiler
...
struct _RygelMprisPluginFactoryLoadActivatablePluginsData {
	...
	RygelPlugin* _tmp10_;
	const gchar* _tmp11_;
	RygelMPRISFreeDesktopDBusObject* _tmp12_;
	GError * _inner_error_;
};
...
static void rygel_mpris_plugin_factory_name_owner_changed (RygelMPRISPluginFactory* self, RygelMPRISFreeDesktopDBusObject* dbus_obj, const gchar* name, const gchar* old_owner, const gchar* new_owner);
static void _rygel_mpris_plugin_factory_name_owner_changed_rygel_mpris_free_desktop_dbus_object_name_owner_changed (RygelMPRISFreeDesktopDBusObject* _sender, const gchar* name, const gchar* old_owner, const gchar* new_owner, gpointer self);
...
void module_init (RygelPluginLoader* loader) {
	...
	g_return_if_fail (loader != NULL);
	{
		...
		RygelMPRISPluginFactory* _tmp3_ = NULL;
		_tmp1_ = loader;
	...
	__catch5_g_io_error:
	{
		...
		g_message (_tmp4_, RYGEL_MPRIS_PLUGIN_MODULE_NAME);
...
RygelMPRISPluginFactory* rygel_mpris_plugin_factory_construct (GType object_type, RygelPluginLoader* loader, GError** error) {
	...
	RygelMPRISFreeDesktopDBusObject* _tmp1_ = NULL;
	RygelMPRISFreeDesktopDBusObject* _tmp2_ = NULL;
	...
	_tmp1_ = g_initable_new (RYGEL_MPRIS_FREE_DESKTOP_TYPE_DBUS_OBJECT_PROXY, NULL, &_inner_error_, "g-flags", G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES, "g-name", RYGEL_MPRIS_PLUGIN_FACTORY_DBUS_SERVICE, "g-bus-type", G_BUS_TYPE_SESSION, "g-object-path", RYGEL_MPRIS_PLUGIN_FACTORY_DBUS_OBJECT, "g-interface-name", "org.freedesktop.DBus", NULL);
...
static gboolean rygel_mpris_plugin_factory_load_plugins_co (RygelMprisPluginFactoryLoadPluginsData* _data_) {
	...
	_state_1:
	_data_->_tmp2_ = NULL;
	_data_->_tmp2_ = rygel_mpris_free_desktop_dbus_object_list_names_finish (_data_->_tmp0_, _data_->_res_, &_data_->_tmp1_, &_data_->_inner_error_);
		...
		for (_data_->service_it = 0; _data_->service_it < _data_->_tmp3__length1; _data_->service_it = _data_->service_it + 1) {
			...
			_data_->service = _data_->_tmp4_;
			{
				...
				if (_data_->_tmp5_) {
					...
					_data_->_state_ = 2;
					rygel_mpris_plugin_factory_load_plugin_n_handle_error (_data_->self, _data_->_tmp12_, rygel_mpris_plugin_factory_load_plugins_ready, _data_);
					return FALSE;
					_state_2:
...
static gboolean rygel_mpris_plugin_factory_load_activatable_plugins_co (RygelMprisPluginFactoryLoadActivatablePluginsData* _data_) {
	...
	_state_1:
	_data_->_tmp2_ = NULL;
	_data_->_tmp2_ = rygel_mpris_free_desktop_dbus_object_list_activatable_names_finish (_data_->_tmp0_, _data_->_res_, &_data_->_tmp1_, &_data_->_inner_error_);
	_data_->services_length1 = 0;
	_data_->_services_size_ = 0;
	_data_->services = _data_->_tmp2_;
	_data_->services_length1 = _data_->_tmp1_;
		...
		for (_data_->service_it = 0; _data_->service_it < _data_->services_length1; _data_->service_it = _data_->service_it + 1) {
			...
			_data_->service = _data_->_tmp3_;
			{
				...
				if (_data_->_tmp4_) {
					...
					_data_->_state_ = 2;
					rygel_mpris_plugin_factory_load_plugin_n_handle_error (_data_->self, _data_->_tmp11_, rygel_mpris_plugin_factory_load_activatable_plugins_ready, _data_);
					return FALSE;
					_state_2:
					rygel_mpris_plugin_factory_load_plugin_n_handle_error_finish (_data_->self, _data_->_res_);
				}
	...
	g_signal_connect (_data_->_tmp12_, "name-owner-changed", (GCallback) _rygel_mpris_plugin_factory_name_owner_changed_rygel_mpris_free_desktop_dbus_object_name_owner_changed, _data_->self);
	_data_->services = (_vala_array_free (_data_->services, _data_->services_length1, (GDestroyNotify) g_free), NULL);
	if (_data_->_state_ == 0) {
		g_simple_async_result_complete_in_idle (_data_->_async_result);
	} else {
...
"it", _tmp12_);
		...
		_tmp14_ = name;
		_tmp15_ = g_str_has_prefix (_tmp14_, RYGEL_MPRIS_PLUGIN_FACTORY_SERVICE_PREFIX);
		if (_tmp15_) {
			const gchar* _tmp16_ = NULL;
			_tmp16_ = name;
			rygel_mpris_plugin_factory_load_plugin_n_handle_error (self, _tmp16_, NULL, NULL);
		}
	}
