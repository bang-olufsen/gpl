/* rygel-import-resource.c generated by valac 0.24.0, the Vala compiler
...
struct _RygelImportResourceRunData {
	...
	GError* _tmp50_;
	const gchar* _tmp51_;
	RygelObjectRemovalQueue* _tmp52_;
...
static void __lambda5_ (RygelImportResource* self);
...
static void ___lambda5__g_cancellable_cancelled (GCancellable* _sender, gpointer self) {
	__lambda5_ ((RygelImportResource*) self);
}
...
RygelImportResource* rygel_import_resource_construct (GType object_type, RygelContentDirectory* content_dir, GUPnPServiceAction* action) {
	...
	g_signal_connect_object (_tmp13_, "cancelled", (GCallback) ___lambda5__g_cancellable_cancelled, self, 0);
...
static gboolean rygel_import_resource_real_run_co (RygelImportResourceRunData* _data_) {
	...
	rygel_object_removal_queue_dequeue (_data_->_tmp20_, (RygelMediaObject*) _data_->_tmp21_);
	{
		...
		_data_->_tmp28_ = _data_->_tmp32_;
		if (_data_->_inner_error_ != NULL) {
			_g_object_unref0 (_data_->source_file);
			goto __catch50_g_error;
		}
		_data_->_tmp33_ = NULL;
		_data_->_tmp33_ = _data_->_tmp28_;
		_data_->_tmp28_ = NULL;
		...
		g_debug ("rygel-import-resource.vala:155: Importing resource from %s to %s", _data_->_tmp45_, _data_->_tmp49_);
	...
	__catch50_g_error:
	{
		...
		g_warning ("rygel-import-resource.vala:161: %s", _data_->_tmp51_);
		_data_->self->status = RYGEL_TRANSFER_STATUS_ERROR;
		_data_->_tmp52_ = NULL;
		...
		_data_->_state_ = 4;
		rygel_object_removal_queue_remove_now (_data_->_tmp52_, (RygelMediaObject*) _data_->_tmp53_, _data_->_tmp55_, rygel_import_resource_run_ready, _data_);
		return FALSE;
		_state_4:
...
static gboolean rygel_import_resource_fetch_item_co (RygelImportResourceFetchItemData* _data_) {
	...
	if (_data_->_tmp14_ == NULL) {
		...
		_data_->_tmp13_ = !G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp15_, RYGEL_TYPE_MEDIA_ITEM);
	}
	if (_data_->_tmp13_) {
		_data_->_tmp12_ = TRUE;
	} else {
	...
	if (_data_->_tmp12_) {
		...
		_data_->_tmp23_ = rygel_media_item_get_place_holder (G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp22_, RYGEL_TYPE_MEDIA_ITEM) ? ((RygelMediaItem*) _data_->_tmp22_) : NULL);
		...
		if (!_data_->_tmp24_) {
			...
			_data_->_tmp31_ = NULL;
			_data_->_tmp31_ = _data_->_tmp30_->uris;
			_data_->_tmp32_ = 0;
	...
	_data_->_tmp38_ = NULL;
	_data_->_tmp38_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp37_, RYGEL_TYPE_MEDIA_ITEM) ? ((RygelMediaItem*) _data_->_tmp37_) : NULL);
...
static void rygel_import_resource_got_headers_cb (RygelImportResource* self, SoupMessage* message) {
	...
	if (_tmp3_) {
		...
		self->status = RYGEL_TRANSFER_STATUS_ERROR;
		{
			...
			_tmp11_ = self->priv->output_stream;
			_tmp12_ = rygel_state_machine_get_cancellable ((RygelStateMachine*) self);
			_tmp13_ = _tmp12_;
			g_output_stream_close ((GOutputStream*) _tmp11_, _tmp13_, &_inner_error_);
			if (_inner_error_ != NULL) {
				goto __catch52_g_error;
			}
			_tmp14_ = self->priv->item;
			_tmp15_ = ((RygelMediaObject*) _tmp14_)->uris;
			...
			_tmp22_ = _tmp21_;
			g_file_delete (_tmp20_, _tmp22_, &_inner_error_);
			if (_inner_error_ != NULL) {
		...
		_tmp23_ = message;
		g_object_get (_tmp23_, "status-code", &_tmp24_, NULL);
		_tmp25_ = _tmp24_;
		_tmp26_ = soup_status_get_phrase (_tmp25_);
		_tmp27_ = g_strdup (_tmp26_);
		phrase = _tmp27_;
		_tmp28_ = message;
		g_object_get (_tmp28_, "status-code", &_tmp29_, NULL);
		_tmp30_ = _tmp29_;
		if (_tmp30_ == ((guint) 404)) {
			GUPnPServiceAction* _tmp31_ = NULL;
			const gchar* _tmp32_ = NULL;
			_tmp31_ = self->priv->action;
			_tmp32_ = phrase;
			gupnp_service_action_return_error (_tmp31_, (guint) 714, _tmp32_);
		} else {
			GUPnPServiceAction* _tmp33_ = NULL;
			const gchar* _tmp34_ = NULL;
			_tmp33_ = self->priv->action;
			_tmp34_ = phrase;
			gupnp_service_action_return_error (_tmp33_, (guint) 715, _tmp34_);
		}
		_g_free0 (phrase);
	}
}
...
static void rygel_import_resource_finished_cb (RygelImportResource* self, SoupMessage* message) {
	...
	if (_tmp0_ == RYGEL_TRANSFER_STATUS_IN_PROGRESS) {
		...
		if (!_tmp1_) {
			...
			_tmp14_ = phrase;
			gupnp_service_action_return_error (_tmp13_, (guint) 714, _tmp14_);
			_g_free0 (phrase);
		}
	}
	_tmp15_ = self->priv->run_callback;
	_tmp15__target = self->priv->run_callback_target;
