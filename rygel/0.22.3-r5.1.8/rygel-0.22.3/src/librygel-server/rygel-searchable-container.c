/* rygel-searchable-container.c generated by valac 0.24.0, the Vala compiler
...
static gboolean rygel_searchable_container_simple_search_co (RygelSearchableContainerSimpleSearchData* _data_) {
	...
	if (_data_->_tmp39_) {
		...
		if (_data_->_tmp46_ == ((guint) 0)) {
			_data_->_tmp45_ = (guint) 0;
		} else {
			_data_->_tmp47_ = 0U;
			_data_->_tmp47_ = _data_->limit;
			_data_->_tmp48_ = NULL;
			_data_->_tmp48_ = _data_->_result_;
			_data_->_tmp49_ = 0;
			_data_->_tmp49_ = gee_abstract_collection_get_size ((GeeCollection*) _data_->_tmp48_);
			_data_->_tmp50_ = 0;
			_data_->_tmp50_ = _data_->_tmp49_;
			_data_->_tmp45_ = _data_->_tmp47_ - _data_->_tmp50_;
		}
		_data_->child_limit = _data_->_tmp45_;
		_data_->_tmp51_ = NULL;
		_data_->_tmp51_ = _data_->expression;
		_data_->_tmp52_ = NULL;
		_data_->_tmp52_ = _data_->children;
		_data_->_tmp53_ = 0U;
		_data_->_tmp53_ = _data_->child_limit;
		_data_->_tmp54_ = NULL;
		_data_->_tmp54_ = _data_->sort_criteria;
		_data_->_tmp55_ = NULL;
		_data_->_tmp55_ = _data_->cancellable;
		_data_->_state_ = 2;
		rygel_searchable_container_search_in_children (_data_->self, _data_->_tmp51_, _data_->_tmp52_, _data_->_tmp53_, _data_->_tmp54_, _data_->_tmp55_, rygel_searchable_container_simple_search_ready, _data_);
		return FALSE;
		_state_2:
		_data_->_tmp56_ = NULL;
		_data_->_tmp56_ = rygel_searchable_container_search_in_children_finish (_data_->self, _data_->_res_, &_data_->_inner_error_);
		_data_->child_results = _data_->_tmp56_;
		if (_data_->_inner_error_ != NULL) {
			g_simple_async_result_set_from_error (_data_->_async_result, _data_->_inner_error_);
			g_error_free (_data_->_inner_error_);
		...
		_data_->_tmp58_ = NULL;
		_data_->_tmp58_ = _data_->child_results;
		gee_array_list_add_all ((GeeArrayList*) _data_->_tmp57_, (GeeCollection*) _data_->_tmp58_);
		_g_object_unref0 (_data_->child_results);
	}
	_data_->_tmp59_ = 0U;
	_data_->_tmp59_ = _data_->max_count;
	if (_data_->_tmp59_ > ((guint) 0)) {
		...
		_data_->total_matches = (guint) _data_->_tmp62_;
	}
	_data_->_tmp63_ = 0U;
	...
	if (_data_->_tmp63_ >= ((guint) _data_->_tmp66_)) {
		_data_->_tmp67_ = NULL;
		_data_->_tmp67_ = rygel_media_objects_new ();
	...
	_data_->_tmp70_ = 0;
	_data_->_tmp70_ = gee_abstract_collection_get_size ((GeeCollection*) _data_->_tmp69_);
	_data_->_tmp71_ = 0;
	...
	if (_data_->_tmp71_ > 0) {
		...
		if (_data_->_tmp73_ > ((guint) 0)) {
			...
			_data_->_tmp72_ = _data_->_tmp74_ > ((guint) 0);
		}
		_data_->_tmp68_ = _data_->_tmp72_;
	} else {
	...
	if (_data_->_tmp68_) {
		...
		_data_->_tmp91_ = NULL;
		_data_->_tmp91_ = G_TYPE_CHECK_INSTANCE_TYPE (_data_->_tmp90_, RYGEL_TYPE_MEDIA_OBJECTS) ? ((RygelMediaObjects*) _data_->_tmp90_) : NULL;
		if (_data_->_tmp91_ == NULL) {
			_g_object_unref0 (_data_->_tmp90_);
		}
		_data_->result = _data_->_tmp91_;
		_g_object_unref0 (_data_->children);
		_g_object_unref0 (_data_->_result_);
		if (_data_->_state_ == 0) {
			g_simple_async_result_complete_in_idle (_data_->_async_result);
