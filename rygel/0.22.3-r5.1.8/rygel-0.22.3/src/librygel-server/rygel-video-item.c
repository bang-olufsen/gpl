/* rygel-video-item.c generated by valac 0.24.0, the Vala compiler
...
static void rygel_video_item_real_add_resources (RygelMediaItem* base, GUPnPDIDLLiteItem* didl_item, gboolean allow_internal, GError** error) {
	...
	g_return_if_fail (didl_item != NULL);
	{
		...
		while (TRUE) {
			...
			if (_tmp14_) {
				...
				_tmp17_ = subtitle;
				_tmp18_ = didl_item;
				rygel_subtitle_add_didl_node (_tmp17_, _tmp18_);
			}
			_g_free0 (protocol);
			_rygel_subtitle_unref0 (subtitle);
		}
	...
	_tmp19_ = didl_item;
	_tmp20_ = allow_internal;
	RYGEL_MEDIA_ITEM_CLASS (rygel_video_item_parent_class)->add_resources ((RygelMediaItem*) G_TYPE_CHECK_INSTANCE_CAST (self, RYGEL_TYPE_AUDIO_ITEM, RygelAudioItem), _tmp19_, _tmp20_, &_inner_error_);
	if (_inner_error_ != NULL) {
		g_propagate_error (error, _inner_error_);
		return;
	}
	_tmp21_ = didl_item;
	_tmp22_ = allow_internal;
	rygel_visual_item_add_thumbnail_resources ((RygelVisualItem*) self, _tmp21_, _tmp22_, &_inner_error_);
	if (_inner_error_ != NULL) {
		g_propagate_error (error, _inner_error_);
		return;
	}
...
static GUPnPDIDLLiteObject* rygel_video_item_real_serialize (RygelMediaObject* base, RygelSerializer* serializer, RygelHTTPServer* http_server, GError** error) {
	...
	if (_tmp3_) {
		...
		GUPnPDIDLLiteContributor* _tmp6_ = NULL;
		GUPnPDIDLLiteContributor* _tmp7_ = NULL;
		const gchar* _tmp8_ = NULL;
		_tmp6_ = gupnp_didl_lite_object_add_author (didl_item);
		contributor = _tmp6_;
		_tmp7_ = contributor;
		_tmp8_ = self->priv->_author;
		gupnp_didl_lite_contributor_set_name (_tmp7_, _tmp8_);
